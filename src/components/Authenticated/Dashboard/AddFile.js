          ...
          {uploadingFiles.map((file) => (
            <section key={file.id}>
              <div className="flex justify-between mt-3">
                <p className="text-black text-sm ml-3 font-semibold">
                  Uploading Files <span> {Math.round(file.progress)}% </span>
                </p>
                <p
                  className="text-googleBtnText text-sm font-semibold cursor-pointer select-none mr-3"
                  onClick={() =>
                    setShowUploadingFiles((showUploading) => !showUploading)
                  }
                >
                  {" "}
              ...
              <div className="flex justify-between mt-4">
                <p className="truncate" style={{ width: "150px" }}>
                  <i className="fa-solid fa-file ml-4 mr-2"></i>{" "}
                  <span className=" text-black"> {file.name} </span>
                </p>

                {file.error && (
                  <div className="flex pt-1 mr-4">
                    <p className="text-red-600 text-sm mx-2">Error</p>
                ...
                {file.progress === 100 && (
                  <i className="fa-solid fa-check text-progressbar cursor-pointer mr-4 mt-1"></i>
                )}
              </div>
